...
function saveProgress() {
  try {
    ...
    localStorage.setItem(
      "ai103_progress_" + QUIZ_FILE,
      JSON.stringify(summary),
    );
    if (
      answeredCount === quizData.length &&
      answeredCount > 0 &&
      !historyLoggedThisLoad
    ) {
...
function collectDisplays() {
  ...
  quizData.forEach((q) => {
    const entry = {};
    let has = false;
    if ((q.type === "single" || q.type === "multi") && q._d) {
      entry.d = q._d;
      has = true;
    }
    if (q.type === "match" && q._poolDisplay) {
      entry.pool = q._poolDisplay;
      has = true;
    }
    if (q.type === "order" && q._display) {
      entry.order = q._display;
      has = true;
    }
  ...
  quizData.forEach((q) => {
    ...
    if (entry.blanksOptions) {
      q.blanks.forEach((b, i) => {
        if (entry.blanksOptions[i]) b._displayOptions = entry.blanksOptions[i];
      });
    }
  });
}
...
function resetQuiz() {
  if (
    !confirm("Làm lại từ đầu? Toàn bộ đáp án đã chọn cho quiz này sẽ bị xoá.")
  )
    return;
  try {
...
const _hasValidSavedState =
  _savedState &&
  Array.isArray(_savedState.answered) &&
  _savedState.answered.length === quizData.length;

let current =
  _hasValidSavedState &&
  typeof _savedState.current === "number" &&
  _savedState.current >= 0 &&
  _savedState.current < quizData.length
    ? _savedState.current
    : 0;
let answered = _hasValidSavedState
  ? _savedState.answered.slice()
  : new Array(quizData.length).fill(null); // null | true | false (correct?)
let userSel = _hasValidSavedState
  ? _savedState.userSel.map((sel, i) =>
      deserializeUserSel(sel, quizData[i].type),
    )
...
function formatExplain(text) {
  ...
  var parts = text.split(
    /(?<=\.)\s+(?=[A-Z0-9ÀÁẢÃẠĂẰẲẴẶẤẦẨẪẬÂÈÉẺẼẸÊỀẾỂỄỆÌÍỈĨỊÒÓỎÕỌÔỒỐỔỖỘƠỜỚỞỠỢÙÚỦŨỤƯỪỨỬỮỰỲÝỶỸỴĐ])/u,
  );
...
function render() {
  ...
  if (q.type === "single" || q.type === "multi") {
    ...
    d.options.forEach((opt, i) => {
      html +=
        ...
        i +
        ')"><div class="mark"></div><div class="label">' +
        opt +
        "</div></div>";
    });
    html += "</div>";
    ...
    q.statements.forEach((s, i) => {
      html +=
        '<div class="statement" data-i="' +
        i +
        '"><div class="stext">' +
        s.text +
        '</div><div class="tfbtns">' +
        '<div class="tfbtn" data-val="true" onclick="setTF(' +
        i +
        ',true)">True / Đúng</div>' +
        '<div class="tfbtn" data-val="false" onclick="setTF(' +
        i +
        ',false)">False / Sai</div>' +
        "</div></div>";
    });
    ...
    q.rows.forEach((r, i) => {
      html +=
        '<div class="matchrow" data-i="' +
        i +
        '"><div class="desc">' +
        r.desc +
        '</div><select onchange="setMatch(' +
        i +
        ',this.value)"><option value="">-- chọn --</option>';
      q._poolDisplay.forEach((p) => {
    ...
    q._display.forEach((text, i) => {
      html +=
        '<div class="orderitem" data-text="' +
        encodeURIComponent(text) +
        '"><select onchange="setOrder(this)"><option value="">#</option>';
      for (let n = 1; n <= q.correctOrder.length; n++)
    ...
    q.blanks.forEach((b, i) => {
      ...
      html +=
        '<div class="fillrow" data-i="' +
        i +
        '">' +
        (b.prefix ? b.prefix + " " : "") +
        '<select onchange="setFill(' +
        i +
        ',this.value)"><option value="">-- chọn --</option>';
      b._displayOptions.forEach((o) => {
  ...
  html +=
    '<div class="explain" id="explainBox"><b>Giải thích:</b><br><br>' +
    formatExplain(q.explain) +
    "</div>";
  html += '<div class="actions">';
  html +=
    '<button class="act btn-check" id="checkBtn" onclick="checkAnswer()">Kiểm tra</button>';
  html +=
    '<button class="act btn-copy" onclick="copyPrompt()">📋 Copy prompt hỏi AI</button>';
  html += "</div>";
  html +=
    '<div class="navrow"><button onclick="goPrev()" ' +
    (current === 0 ? "disabled" : "") +
    '>&larr; Câu trước</button><button onclick="goNext()" ' +
    (current === quizData.length - 1 ? "disabled" : "") +
    ">Câu sau &rarr;</button></div>";
  html += "</div>";
...
function toggleOption(i) {
  ...
  if (!userSel[current])
    userSel[current] = q.type === "multi" ? new Set() : null;
...
function setTF(i, val) {
  ...
  stmt
    .querySelector('.tfbtn[data-val="' + val + '"]')
...
function checkAnswer() {
  ...
  if (q.type === "single") {
    ...
    if (sel.size === 0) {
      alert("Hãy chọn các đáp án.");
      return;
    }
    const correctSet = new Set(d.correct);
    isCorrect =
      sel.size === correctSet.size && [...sel].every((x) => correctSet.has(x));
    const els = document.querySelectorAll("#optList .option");
    els.forEach((e, i) => {
    ...
    const sel = userSel[current] || {};
    if (Object.keys(sel).length < q.statements.length) {
      alert("Hãy chọn True/False cho tất cả.");
      return;
    }
    isCorrect = true;
    ...
    q.statements.forEach((s, i) => {
      ...
      if (!ok) {
        const wrongBtn = stmts[i].querySelector(
          '.tfbtn[data-val="' + userVal + '"]',
        );
    ...
    const sel = userSel[current] || {};
    if (Object.keys(sel).length < q.rows.length) {
      alert("Hãy chọn cho tất cả các dòng.");
      return;
    }
    isCorrect = true;
    ...
    const sel = userSel[current] || {};
    if (
      Object.keys(sel).length < q.correctOrder.length ||
      Object.values(sel).some((v) => !v)
    ) {
      alert("Hãy chọn số thứ tự cho tất cả.");
      return;
    }
    isCorrect = true;
    ...
    const sel = userSel[current] || {};
    if (
      Object.keys(sel).length < q.blanks.length ||
      Object.values(sel).some((v) => !v)
    ) {
      alert("Hãy chọn cho tất cả các ô trống.");
      return;
    }
    isCorrect = true;
...
function restoreState(q) {
  ...
  if (q.type === "single" || q.type === "multi") {
    ...
    const correctSet = new Set(
      Array.isArray(d.correct) ? d.correct : [d.correct],
    );
    els.forEach((e, i) => {
      e.onclick = null;
      const userHas =
        q.type === "multi" ? sel && sel.has && sel.has(i) : sel === i;
      if (correctSet.has(i)) e.classList.add("correct");
...
function buildPromptText(q) {
  let body =
    "Mục tiêu là để hiểu câu hỏi và kiến thức AI-103, vậy nên giải thích ngắn gọn, dễ hiểu bằng tiếng Việt (có thể cung cấp flows và ref từ Microsoft page miễn sao dễ hiểu và dễ nhơ) câu hỏi AI-103 sau — vì sao đáp án đúng là đúng và các lựa chọn khác sai ở đâu:\n\n";
  body += q.text + "\n";
  ...
  if (q.type === "single" || q.type === "multi") {
    ...
    body +=
      "Đáp án đúng: " +
      correctArr.map((i) => String.fromCharCode(65 + i)).join(", ") +
      "\n";
  } else if (q.type === "tf") {
    q.statements.forEach((s, i) => {
      body +=
        i + 1 + ". " + s.text + " → " + (s.answer ? "True" : "False") + "\n";
    });
  } else if (q.type === "match") {
    q.rows.forEach((r, i) => {
...
function copyPrompt() {
  ...
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard
      .writeText(text)
      .then(doToast)
      .catch(() => fallbackCopy(text, doToast));
  } else {
